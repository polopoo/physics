...
## Почему не использовался HC18 как основной набор

[HC18](https://hc18.grand-challenge.org/) содержит отдельные 2D УЗИ стандартного сечения головы. Это снимки разных исследований, а не упорядоченный набор срезов одной головы. Их нельзя просто сложить друг за другом и получить правильный объём.

В папках `hc18/` и `hc18_test/` есть отдельные эксперименты: по контуру головы строится эллипсоид. Его глубина задаётся предположением, поэтому это только геометрическая демонстрация, а не настоящая 3D-реконструкция.
...
Получился воспроизводимый пример цепочки `2D УЗИ-срезы → 3D-объём → STL`. Техническая часть работает, модели открываются и проходят проверку геометрии. Главный недостаток — использование срезов готового атласа с известным положением, поэтому работа не решает полную задачу реконструкции конкретного эмбриона по обычным 2D-снимкам.
